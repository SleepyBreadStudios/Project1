...
public class SnowGolemBehavior : EnemyBehavior
{
    ...
    [SerializeField]
    private GameObject spikeLead = null;
    ...
    public void ShootServerRpc()
    {
        if (GameObject.FindWithTag("Player") != null)
        {
            ...
            GameObject spikeInstance = Instantiate(spikeLead, appleLoc, Quaternion.identity);

            // IMPORTANT: get network to recognize object
            spikeInstance.GetComponent<NetworkObject>().Spawn(true);

            spikeInstance.transform.rotation = Quaternion.LookRotation(Vector3.forward, playerLoc - appleLoc);
        }
    }
}